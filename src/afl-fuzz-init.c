...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  fn = alloc_printf("%s/queue2", afl->out_dir);
  if (delete_files(fn, CASE_PREFIX)) { goto dir_cleanup_failed; }
  ck_free(fn);
  ...
  fn = alloc_printf("%s/FRIEND/argvs_ratio", afl->out_dir);
  if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/FRIEND/num_func_calls", afl->out_dir);
  if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/FRIEND/argvs_selected", afl->out_dir);
  if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
  ck_free(fn);
  ...
  fn = alloc_printf("%s/FRIEND/argv_rels", afl->out_dir);
  if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
  ck_free(fn);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  tmp = alloc_printf("%s/queue2", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
  ...
  tmp = alloc_printf("%s/tc_graph", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
...
void setup_testcase_shmem(afl_state_t *afl) {
  ...
  u8 *map = afl_shm_init(afl->shm_fuzz, MAX_FILE + sizeof(u32), 1, 0);
