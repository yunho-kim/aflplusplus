...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault, u32 parent_id1, u32 parent_id2, struct argv_word_entry * args, u32 argv_idx) {
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    if (afl->funcrel_file_mut) {
      u32 parent_idx = afl->syncing_party ? (u32) -1 :
        (parent_id2 == (u32) -1) ? parent_id1 : (u32) -1;
    
      update_tc_graph_and_branch_cov(afl, afl->queued_paths - 1, parent_idx, mem, len);
    }

    update_only_branch_cov(afl, afl->queued_paths - 1, mem, len);
  }
